...
export async function getLandingPageData() {
  ...
  const [eventsRes, collabsRes, usersRes, clubsRes] = await Promise.all([
    supabase.from('events').select('*, clubs:club_id(name, initials, accent_color)').gte('event_date', today).order('event_date', { ascending: true }),
    supabase.from("collabs").select("*, author:author_id(id, full_name, stream, batch_year, avatar_url)").eq("is_open_collab", true).order("created_at", { ascending: false }).limit(4),
    supabase.from("users").select("id, stream, interests").not("stream", "is", null),
    supabase.from("clubs").select("id, name, initials, accent_color, description, follower_count, category, type, tags").order("follower_count", { ascending: false }).limit(10)
  ]);

  // Process Events
  let events = (eventsRes.data || []).map((e: any) => ({
    ...e,
    event_date: e.event_date || e.created_at,
    location: e.location || e.metadata?.location || e.metadata?.event_location || null,
    start_time: e.metadata?.event_time || e.metadata?.start_time || null,
  ...
  // Prioritize SIH
  const sihIndex = events.findIndex((e: any) => e.title && e.title.toLowerCase().includes('smart india hackathon'));
  let finalEvents = [];
  if (sihIndex !== -1) {
    finalEvents.push(events[sihIndex]);
    events.splice(sihIndex, 1);
  }
  finalEvents = [...finalEvents, ...events].slice(0, 4);

  // Process Users to build Streams
  const btechStreams = ["Computer Science and Engineering", "Electronics and Communication Engineering", "Electrical and Computer Engineering", "Mechanical Engineering", "Chemical Engineering", "Biotechnology", "Civil Engineering"];
  const bscStreams = ["Chemistry", "Mathematics", "Physics"];
  const commerceStreams = ["Economics", "Bachelor of Management Studies (BMS)", "B.Sc. (Research) in Economics and Finance", "BA (Research) in Business and Management Studies", "MBA"];
  const socialStreams = ["English", "History", "Sociology", "International Relations and Governance Studies", "Psychology"];

  const streamsMap: Record<string, any[]> = { btech: [], bsc: [], commerce: [], social: [] };
  const userList = usersRes.data || [];

  const processStreamGroup = (groupKey: string, streamNames: string[]) => {
    const colors = ["var(--lime)", "var(--cyan)", "var(--purple)", "var(--coral)"];
    
    streamNames.forEach((sName, i) => {
      const sUsers = userList.filter((u: any) => u.stream === sName);
      if (sUsers.length > 0) {
        const interestCounts: Record<string, number> = {};
        sUsers.forEach((u: any) => {
          if(u.interests) u.interests.forEach((intr: string) => {
            interestCounts[intr] = (interestCounts[intr] || 0) + 1;
          });
        });
        const topTags = Object.entries(interestCounts).sort((a,b) => b[1]-a[1]).slice(0,3).map(arr => arr[0]);
        streamsMap[groupKey].push({
          name: sName.replace(" and Engineering", "").replace(" and Communication", ""),
          count: `${sUsers.length} students`,
          color: colors[i % colors.length],
          tags: topTags.length > 0 ? topTags : ["Tech", "Campus", "Events"]
        });
      }
    });
  };

  processStreamGroup('btech', btechStreams);
  processStreamGroup('bsc', bscStreams);
  processStreamGroup('commerce', commerceStreams);
  processStreamGroup('social', socialStreams);

  if (streamsMap.btech.length === 0) streamsMap.btech.push({ name: "Engineering", count: "0 students", color: "var(--lime)", tags: ["Welcome"] });
  if (streamsMap.bsc.length === 0) streamsMap.bsc.push({ name: "Sciences", count: "0 students", color: "var(--cyan)", tags: ["Welcome"] });
  if (streamsMap.commerce.length === 0) streamsMap.commerce.push({ name: "Business", count: "0 students", color: "var(--purple)", tags: ["Welcome"] });
  if (streamsMap.social.length === 0) streamsMap.social.push({ name: "Humanities", count: "0 students", color: "var(--coral)", tags: ["Welcome"] });

  return { 
    initialEvents: finalEvents, 
    initialCollabs: collabsRes.data || [], 
    initialStreams: streamsMap, 
