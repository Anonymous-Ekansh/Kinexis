    ...
    async function load() {
      ...
      try {
        const { data } = await supabase
          .from("users")
          .select("id, full_name, avatar_url, stream, batch_year, bio, interests")
          .eq("id", userId)
          .maybeSingle();

        setProfile(data || { id: userId, full_name: "User", avatar_url: null, stream: "", bio: "", interests: [] });
    ...
    <div className="msg-modal-overlay" onClick={onClose}>
      <div className="msg-profile-card" onClick={e => e.stopPropagation()}>
        ...
        <div className="msg-profile-top">
          ...
          <div className="msg-profile-name">{profile?.full_name || (loading ? 'Loading...' : 'User')}</div>
